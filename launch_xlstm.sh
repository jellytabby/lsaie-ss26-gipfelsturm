#!/bin/bash
#
# Usage: ./launch_xlstm.sh <mode> <model_size> [steps] [nodes]
#
...
#   - Runs pretrain_xlstm.py instead of pretrain_gpt.py
#   - Installs the xlstm package instead of mamba-ssm / causal-conv1d
#   - Drops --position-embedding-type rope  (xLSTM is position-free by gates)
#   - Drops --group-query-attention / --num-query-groups  (no KV heads in xLSTM)
#   - Uses MASTER_PORT=25680 to avoid colliding with GPT (25678) / Mamba (25679)
#
# Examples:  ./launch_xlstm.sh throughput 760m
#            ./launch_xlstm.sh throughput 8b 50 1
#            ./launch_xlstm.sh train 760m 5000
#            ./launch_xlstm.sh train 1.5b 3000 8

set -euo pipefail

source "$(dirname "$0")/config.sh"

MODE=${1:?Usage: ./launch_xlstm.sh <mode> <model_size> [steps] [nodes]}
MODEL_SIZE=${2:?Usage: ./launch_xlstm.sh <mode> <model_size> [steps] [nodes]}
...
case $MODE in
    throughput)
        ...
        ;;
    train)
        TRAINING_STEPS=${3:?Usage: ./launch_xlstm.sh train <model_size> <steps> [nodes]}
        NODES=${4:-4}
        TIME=02:30:00
        EVAL_INTERVAL=1000
        EVAL_ITERS=10
...
case $MODEL_SIZE in
    ...
    760m)
        ...
        ;;
    1.5b)
        NUM_LAYERS=48; HIDDEN=1600; FFN=4352;  HEADS=20
        MBS=1
        ;;
    3b)
...
GBS=256
SEQ_LEN=16384
JOB_NAME="gipfel-xlstm-${MODE}-${MODEL_SIZE}-${TRAINING_STEPS}s-${NODES}n"
...
EXP_NAME=xlstm-${MODE}-${MODEL_SIZE}-\${SLURM_NNODES}n
...
TRAINING_CMD="torchrun ${TORCHRUN_ARGS[@]} $MEGATRON_LM_DIR/pretrain_xlstm.py \
    ${TRANSFORMER_ENGINE_ARGS[@]} \
    ${NETWORK_SIZE_ARGS[@]} \
    ${TRAINING_ARGS[@]} \
    ${REGULARIZATION_ARGS[@]} \
